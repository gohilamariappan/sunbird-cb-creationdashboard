...
import { AUTHORING_CONTENT_BASE } from '../routes/my-content/constants/apiEndpoints'
import { ICON_TYPE } from '../routes/my-content/constants/icons'
import { MIME_TYPE } from '../routes/my-content/constants/mimeType'
...
export class AccessControlService {
  ...
  getCategoryType(content: NSContent.IContentMeta | ISearchContent): string {
    switch (this.getCategory(content)) {
      case 'Resource':
        return content.contentType || content.resourceType || 'Resource'
      case 'Collection':
        return content.contentType || 'Module'
      case 'Course':
        return content.contentType || 'Course'
      case 'Learning Path':
        return content.contentType || 'Program'
      default:
